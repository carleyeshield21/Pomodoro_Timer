...
def reset_timer():
    ...
    timer_title.config(text='Tay-Mer')
    chick_mark.config(text='')
    global reps
    reps = 0
...
def count_down(count):
    ...
    if count > 0:
        ...
        is_start_count()
        mark = ''
        for tsik in range(math.floor(reps/2)):
            mark += '✅'
        chick_mark.config(text=mark,fg=GREEN,background=YELLOW)
...
reset_button = Button(text='Reset',font=("Regular",25,'bold'),highlightthickness=0,command=reset_timer)
reset_button.grid(row=2,column=2)

# chick_mark = Label(text='✅',foreground=RED,background=GREEN,font=('Regular',25),highlightthickness=0)
# chick_mark = Label(foreground=RED,font=('Regular',25),highlightthickness=0)
chick_mark = Label(font=('Regular',25))
